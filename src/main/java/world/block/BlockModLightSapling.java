...
public class BlockModLightSapling extends BlockSapling implements IGrowable
{
	...
	public BlockModLightSapling(String blockname, String textureName)
	{
		...
		this.setCreativeTab(MagicMod.tabCrops);
		this.setBlockName("light_Sapling");
	...
	public void registerBlockIcons(IIconRegister iconRegister)
	{
		...
		{
			textures[i] = iconRegister.registerIcon("mm:" + saplings[i]);
		}
	}
	...
	public boolean isValidPosition(World world, int x, int y, int z, int metadata)
	{
		...
		{
		default:
			return block == Blocks.grass || block == Blocks.grass || block.canSustainPlant(world, x, y - 1, z, ForgeDirection.UP, this);
		}
	}
	...
	public boolean canSustainPlant(IBlockAccess world, int x, int y, int z, ForgeDirection direction, IPlantable plantable) {
		...
		if (world.getBlock(x, y, z) == MagicMod.Grass) {
			return true;
		}
		else {
			return false;
	...
	public void onPlantGrow(World world, int x, int y, int z, int sourceX, int sourceY, int sourceZ) {
		if (this == Blocks.dirt || this == MagicMod.Grass)
		{
			world.setBlock(x, y, z, MagicMod.Grass, 0, 2);
		}
	}
}
